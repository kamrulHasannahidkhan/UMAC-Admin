...
const navItems: NavItem[] = [
  ...
  {
    label: "About Page",
    href: "/about-page", // Added route for the parent About Page
    icon: BookOpenCheck,
    children: [
  ...
  {
    label: "Facilities",
    href: "/facilities", // Optional: Add route if main page exists
    icon: Building2,
    children: [
  ...
  {
    label: "Notice & Media",
    href: "/notice-media", // Optional: Add route if main page exists
    icon: Bell,
    children: [
  ...
  {
    label: "Admission",
    href: "/admission", // Optional: Add route if main page exists
    icon: ClipboardList,
    children: [
  ...
  {
    label: "Contact Us",
    href: "/contact", // Optional: Add route if main page exists
    icon: Phone,
    children: [
...
export default function Sidebar() {
  ...
  useEffect(() => {
    const activeGroup = navItems.find((item) =>
      item.children?.some((c) => c.href === pathname) || item.href === pathname
    );
          ...
          return (
            <div key={item.label} className="w-full">
              {hasChildren ? (
                ...
                >
                  {/* Left Side: Direct Link (or toggle button if no href) */}
                  {item.href ? (
                    <Link
                      href={item.href}
                      className="flex-1 flex items-center gap-3 px-3 py-2.5 text-xs font-semibold"
                    >
                      <Icon
                        size={18}
                        className={
                    ...
                    <button
                      ...
                      className="flex-1 flex items-center gap-3 px-3 py-2.5 text-xs font-semibold text-left"
                    >
                      <Icon
                        size={18}
                        className={isChildActive ? "text-emerald-400" : "text-slate-400"}
                      />
                      <span>{item.label}</span>
                    </button>
                  )}

                  {/* Right Side: Chevron Button for Accordion Toggle */}
                  <button
                    type="button"
                    onClick={() => toggleGroup(item.label)}
                    aria-label={`Toggle ${item.label} submenu`}
                    className="p-2.5 text-slate-500 hover:text-slate-200 focus:outline-none"
                  >
                    <ChevronDown
                      size={15}
