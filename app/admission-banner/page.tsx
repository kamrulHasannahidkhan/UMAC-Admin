...
export default function AdmissionBannerAdmin() {
  const [banner, setBanner] = useState<AdmissionBannerContent | null>(null);
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/admission-banner", { method, body: formData });
      const json = await res.json();
      if (json.success) {
        setBgImage(null);
        setLogoImage(null);
  ...
  return (
    <main className="max-w-3xl mx-auto px-6 py-10">
      <h1 className="text-2xl font-semibold mb-6">Admission Banner Admin</h1>
