...
export async function GET() {
  try {
    await connectDB();
    const docs = await AdmissionDocument.find().sort({ category: 1, order: 1, createdAt: -1 });
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!title || !date || !category) {
      return withCors(
        NextResponse.json({ success: false, error: "Title, Date and Category are required" }, { status: 400 })
      );
    }
    if (!["papers", "forms", "results"].includes(category)) {
      return withCors(NextResponse.json({ success: false, error: "Invalid category" }, { status: 400 }));
    }

    const bytes = await file.arrayBuffer();
    const buffer = Buffer.from(bytes);

    // PDFs are uploaded as raw resources, not images
