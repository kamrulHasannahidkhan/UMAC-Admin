...
import AdmissionAid from "@/models/AdmissionAid";
...
export async function GET() {
  await connectDB();
  const doc = await AdmissionAid.findOne();
  return withCors(NextResponse.json({ success: true, data: doc }));
}

export async function POST(req: NextRequest) {
  await connectDB();
  const existing = await AdmissionAid.findOne();
  ...
  const uploaded = await new Promise<any>((resolve, reject) => {
    cloudinary.uploader.upload_stream({ folder: "uamc/admission-aid" }, (err, result) => (err ? reject(err) : resolve(result))).end(buffer);
  });

  const doc = await AdmissionAid.create({
...
export async function PUT(req: NextRequest) {
  ...
  const formData = await req.formData();
  const updates: Record<string, any> = { heading: formData.get("heading") ?? existing.heading };
  ...
  const updated = await AdmissionAid.findByIdAndUpdate(existing._id, updates, { new: true });
