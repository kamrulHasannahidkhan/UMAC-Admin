...
export async function GET() {
  await connectDB();
  const doc = await ContactBanner.findOne();
...
export async function POST(req: NextRequest) {
  ...
  const uploaded = await new Promise<any>((resolve, reject) => {
    cloudinary.uploader.upload_stream({ folder: "uamc/contact-banner" }, (err, result) => (err ? reject(err) : resolve(result))).end(buffer);
  });

  const doc = await ContactBanner.create({
...
export async function PUT(req: NextRequest) {
  ...
  const updated = await ContactBanner.findByIdAndUpdate(existing._id, updates, { new: true });
