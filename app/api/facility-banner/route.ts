...
import FacilityBanner from "@/models/FacilityBanner";
...
async function uploadImage(file: File) {
  ...
  return new Promise<any>((resolve, reject) => {
    cloudinary.uploader
      .upload_stream({ folder: "uamc/facility-banner" }, (err, result) => (err ? reject(err) : resolve(result)))
      .end(buffer);
  });
}
...
export async function GET() {
  await connectDB();
  const banner = await FacilityBanner.findOne();
  return withCors(NextResponse.json({ success: true, data: banner }));
}

export async function POST(req: NextRequest) {
  await connectDB();
  const existing = await FacilityBanner.findOne();
  if (existing) {
    return withCors(
      NextResponse.json({ success: false, error: "Facility banner already exists — use edit instead." }, { status: 400 })
    );
  }
  ...
  const banner = await FacilityBanner.create({
...
export async function PUT(req: NextRequest) {
  ...
  const existing = await FacilityBanner.findOne();
  if (!existing) {
    return withCors(NextResponse.json({ success: false, error: "No Facility banner to update yet" }, { status: 404 }));
  }
  ...
  if (bgImage && bgImage.size > 0) {
    if (existing.bgImagePublicId) await cloudinary.uploader.destroy(existing.bgImagePublicId);
  ...
  const logoImage = formData.get("logoImage") as File | null;
  if (logoImage && logoImage.size > 0) {
    if (existing.logoImagePublicId) await cloudinary.uploader.destroy(existing.logoImagePublicId);
  ...
  const updated = await FacilityBanner.findByIdAndUpdate(existing._id, updates, { new: true });
