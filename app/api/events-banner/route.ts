...
import EventsBanner from "@/models/EventsBanner";
...
export async function GET() {
  await connectDB();
  const doc = await EventsBanner.findOne();
  return withCors(NextResponse.json({ success: true, data: doc }));
}

export async function POST(req: NextRequest) {
  await connectDB();
  const existing = await EventsBanner.findOne();
  ...
  const uploaded = await new Promise<any>((resolve, reject) => {
    cloudinary.uploader.upload_stream({ folder: "uamc/events-banner" }, (err, result) => (err ? reject(err) : resolve(result))).end(buffer);
  });

  const doc = await EventsBanner.create({
...
export async function PUT(req: NextRequest) {
  ...
  const updated = await EventsBanner.findByIdAndUpdate(existing._id, updates, { new: true });
