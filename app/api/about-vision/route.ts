...
import AboutVision from "@/models/AboutVision";
...
export async function GET() {
  await connectDB();
  const doc = await AboutVision.findOne();
  return withCors(NextResponse.json({ success: true, data: doc }));
}

export async function POST(req: NextRequest) {
  await connectDB();
  const existing = await AboutVision.findOne();
  ...
  const uploaded = await new Promise<any>((resolve, reject) => {
    cloudinary.uploader.upload_stream({ folder: "uamc/about-vision" }, (err, result) => (err ? reject(err) : resolve(result))).end(buffer);
  });

  const doc = await AboutVision.create({
...
export async function PUT(req: NextRequest) {
  ...
  const updated = await AboutVision.findByIdAndUpdate(existing._id, updates, { new: true });
