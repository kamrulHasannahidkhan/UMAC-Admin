...
import NoticeBanner from "@/models/NoticeBanner";
...
async function uploadImage(file: File) {
  ...
  return new Promise<any>((resolve, reject) => {
    cloudinary.uploader
      .upload_stream({ folder: "uamc/notice-banner" }, (err, result) => (err ? reject(err) : resolve(result)))
      .end(buffer);
  });
}

// Single-document section: GET returns the one Notice Banner doc (or null)
export async function GET() {
  await connectDB();
  const banner = await NoticeBanner.findOne();
...
export async function POST(req: NextRequest) {
  await connectDB();
  const existing = await NoticeBanner.findOne();
  if (existing) {
    return withCors(
      NextResponse.json(
        { success: false, error: "Notice banner already exists — use edit instead." },
        { status: 400 }
      )
    );
  }
  ...
  const banner = await NoticeBanner.create({
...
export async function PUT(req: NextRequest) {
  ...
  const existing = await NoticeBanner.findOne();
  if (!existing) {
    return withCors(NextResponse.json({ success: false, error: "No Notice banner to update yet" }, { status: 404 }));
  }
  ...
  const updated = await NoticeBanner.findByIdAndUpdate(existing._id, updates, { new: true });
