...
import CareerBanner from "@/models/CareerBanner";
...
export async function GET() {
  await connectDB();
  const doc = await CareerBanner.findOne();
  return withCors(NextResponse.json({ success: true, data: doc }));
}

export async function POST(req: NextRequest) {
  await connectDB();
  const existing = await CareerBanner.findOne();
  ...
  const uploaded = await new Promise<any>((resolve, reject) => {
    cloudinary.uploader.upload_stream({ folder: "uamc/career-banner" }, (err, result) => (err ? reject(err) : resolve(result))).end(buffer);
  });

  const doc = await CareerBanner.create({
...
export async function PUT(req: NextRequest) {
  ...
  const updated = await CareerBanner.findByIdAndUpdate(existing._id, updates, { new: true });
