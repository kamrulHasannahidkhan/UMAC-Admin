...
export default function FacilityBannerAdmin() {
  const [banner, setBanner] = useState<FacilityBannerContent | null>(null);
  ...
  const fetchBanner = async () => {
    try {
      const res = await fetch("/api/facility-banner");
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/facility-banner", { method, body: formData });
      const json = await res.json();
      if (json.success) {
        setBgImage(null);
        setLogoImage(null);
  ...
  return (
    <main className="max-w-3xl mx-auto px-6 py-10">
      <h1 className="text-2xl font-semibold mb-6">Facility Banner Admin</h1>
